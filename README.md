...
当前通用大语言模型（LLM）和语音识别（ASR）在处理大众化语料时已较为成熟，但在高度垂直、专业性强且文化壁垒明显的特定领域（如日本将棋、传统手工艺、古籍整理等），仍面临专有名词识别偏差、文化语境理解不足等问题。
...
1. **动态领域字典注入 (Dynamic Glossary Injection)**  
   利用自建的将棋专业术语知识库（`shogi_glossary.json`），在翻译流程中对 LLM 的词汇映射进行引导，减少专有名词误译。

2. **结构化提示词设计 (Prompt Engineering)**  
   针对将棋记谱法和专业解说语境，设计专用 System Prompt，指导模型按照行业标准格式输出（如“7七金”）。

3. **人机协同工作流 (Human-in-the-Loop)**  
   将音视频处理、初步翻译等重复性任务交给机器，同时保留可视化控制台供领域专家进行最终审核与微调。
